...
class WCPBC_Frontend_Pricing {
	...
	private static function init_hooks() {
		...
		add_filter( 'the_posts', array( __CLASS__, 'remove_product_query_filters' ), 20, 2 );
		...
		add_action( 'woocommerce_new_order', array( __CLASS__, 'update_order_meta' ), 10, 2 );
	}
	...
	public static function get_catalog_ordering_args( $args ) {
		if ( isset( $args['meta_key'] ) && '_price' === $args['meta_key'] ) {
			...
			// Since WC 3.1.
			add_filter( 'posts_clauses', array( __CLASS__, 'order_by_price_post_clauses' ), 20, 2 );
		}
	 ...
	 * @return array
	 */
	public static function order_by_price_post_clauses( $args, $wp_query = null ) {
		global $wpdb;
		...
		} elseif ( ! is_null( $wp_query ) && $wp_query->is_main_query() && 'product_query' === $wp_query->get( 'wc_query' ) ) {
	...
	public static function filter_price_post_clauses( $args, $wp_query ) {
		...
		if ( version_compare( WC_VERSION, '3.6', '<' ) || ! $wp_query->is_main_query() || 'product_query' !== $wp_query->get( 'wc_query' ) || ( ! isset( $_GET['max_price'] ) && ! isset( $_GET['min_price'] ) ) ) { // phpcs:ignore WordPress.Security.NonceVerification
			return $args;
		}
	 ...
	 * @return array
	 */
	public static function remove_product_query_filters( $posts, $wp_query ) {
	 ...
	 */
	public static function update_order_meta( $order_id, $order = null ) {
		$base_exchange_rate = wcpbc_the_zone()->get_base_currency_amount( 1 );
		$pricing_zone_data  = wcpbc_the_zone()->get_data();

		// Add metadata.
		$base_exchange_rate_id = update_post_meta( $order_id, '_wcpbc_base_exchange_rate', $base_exchange_rate );
		$pricing_zone_data_id  = update_post_meta( $order_id, '_wcpbc_pricing_zone', $pricing_zone_data );

		if ( ! is_null( $order ) && is_callable( array( $order, 'set_meta_data' ) ) ) {
			// Update the order object. Prevents issues with the Subscription plugin.
			$data = array(
				array(
					'id'    => $base_exchange_rate_id,
					'key'   => '_wcpbc_base_exchange_rate',
					'value' => $base_exchange_rate,
				),
				array(
					'id'    => $pricing_zone_data_id,
					'key'   => '_wcpbc_pricing_zone',
					'value' => $pricing_zone_data,
				),
			);
