...
class WCPBC_Frontend {
	...
	public static function init() {
		...
		add_action( 'wp_loaded', array( __CLASS__, 'maybe_refresh_cart' ), 30 );
	...
	public static function load_scripts() {
		...
		wp_localize_script(
			...
			array(
				...
				'pjax_timeout' => absint( apply_filters( 'wc_price_based_country_pjax_timeout', 200 ) ),
			)
		);
	...
	public static function init_session() {
		...
		if ( $cart_content_total && ! did_action( 'woocommerce_after_calculate_totals' ) ) {
			WC()->cart->calculate_totals();
		} else {
			// Set the cart_hash cookie to force the cart to refresh.
			wc_setcookie( 'woocommerce_cart_hash', self::get_cart_hash( '', true ) );
		}
	}

	/**
	 * Calculate totals if the cart hash does not match with the cookie value. Only for WC 3.6+
	 *
	 * @since 2.0.24
	 */
	public static function maybe_refresh_cart() {
		if ( version_compare( WC_VERSION, '3.6', '<' ) || is_ajax() || is_cart() || is_checkout() || ! isset( $_COOKIE['woocommerce_cart_hash'] ) || ! did_action( 'woocommerce_cart_loaded_from_session' ) || did_action( 'woocommerce_after_calculate_totals' ) ) {
			return;
		}
		$cart_hash   = WC()->cart->get_cart_hash();
		$cookie_hash = isset( $_COOKIE['woocommerce_cart_hash'] ) ? $_COOKIE['woocommerce_cart_hash'] : '';

		if ( $cart_hash !== $cookie_hash ) {
			if ( WC()->cart->is_empty() ) {
