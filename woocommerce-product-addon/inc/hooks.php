...
function ppom_hooks_load_input_scripts( $product, $ppom_id=null ) {
	...
	if( !empty($ppom_conditional_fields) || apply_filters('ppom_enqueue_conditions_js', false)) {
		...
		wp_enqueue_script( "ppom-{$ppom_conditions_script}", PPOM_URL."/js/{$ppom_conditions_script}.js", array('jquery','ppom-inputs'), PPOM_VERSION, true);
...
function ppom_hooks_search_in_order($search_fields){
	...
	$orders = get_posts( array( 'post_type' => 'shop_order', 'post_status' => 'completed', 'numberposts'=>400 ) );
	...
	foreach ($orders as $order_post) {
		...
		foreach( $order->get_items() as $item_id => $item_values ) {
			...
			if( ! isset($ppom_fields_post['fields']) ) continue;
