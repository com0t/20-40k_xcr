...
class PPOM_SettingsFramework {
	...
	*/
    ...
    function __construct(){
	    ...
	    add_action( 'wp_ajax_'.$this->get_config('id').'_settings_panel_action', array($this,'save_settings') );
	...
	public function register_tabs($tabs){
		...
		if (array_intersect_key($tabs, $this->tabs)) return $this;
	...
	public function load_inputs($input_meta){
		...
		self::load_template("/templates/inputs/{$type}.php", array('input_meta' => $input_meta, 'class_ins' => $this));
	}
	...
	public static function reposition_array_element($arr, $find, $move) {
	    ...
	    if (!isset($arr[$find], $arr[$move])) { return $arr; }
    ...
    function save_settings(){
    	...
    	|| ! current_user_can('ppom_options_page')
		) {
			$response = array(
            	'status'  => 'error' , 
        ...
        if (isset($_REQUEST[self::$save_key])) {
            ...
            $response = array(
            	'status'  => 'success', 
            	'message' => __('Settings saved successfully.', $this->get_config('plugin_domain'))
        	);
           
        }else{
            $response = array(
            	'status'  => 'error' , 
            	'message' => __('Settings saved error.', $this->get_config('plugin_domain'))
        	);
        }
    ...
    function ppom_migrate_settings_panel(){
		...
		if ($old_settings == 'yes') {
			...
			wp_redirect( $ppom_settings_url ); exit;
		}else{
	...
	public function border_style(){
		...
		$style = array(
			'none'   => __( 'None', $this->get_config('plugin_domain') ),
			'solid'  => __( 'Solid', $this->get_config('plugin_domain') ),
			'dotted' => __( 'Dotted', $this->get_config('plugin_domain') ),
			'dashed' => __( 'Dashed', $this->get_config('plugin_domain') ),
			'double' => __( 'Double', $this->get_config('plugin_domain') ),
			'groove' => __( 'Groove', $this->get_config('plugin_domain') ),
			'rige'   => __( 'Ridge', $this->get_config('plugin_domain') ),
			'inset'  => __( 'Inset', $this->get_config('plugin_domain') ),
			'outset' => __( 'Outset', $this->get_config('plugin_domain') ),
		);
	...
	public function css_editor_options(){
		...
		$option = array(
			'top'   => array(
				'title' => __( 'Top', $this->get_config('plugin_domain') ),
				'icon'  => 'dashicons dashicons-arrow-up-alt'
			),
			'right'  => array(
				'title' => __( 'Right', $this->get_config('plugin_domain') ),
				'icon'  => 'dashicons dashicons-arrow-right-alt'
			),
			'bottom' => array(
				'title' => __( 'Bottom', $this->get_config('plugin_domain') ),
				'icon'  => 'dashicons dashicons-arrow-down-alt'
			),
			'left' => array(
				'title' => __( 'Left', $this->get_config('plugin_domain') ),
	...
	public function typography_options(){
		...
		$option = array(
			'font-size'   => array(
				'title' => __( 'Font Size', $this->get_config('plugin_domain') ),
			),
			'line-height'  => array(
				'title' => __( 'Line Height', $this->get_config('plugin_domain') ),
			),
			'color' => array(
				'title' => __( 'Font Color', $this->get_config('plugin_domain') ),
			),
		);
	...
	public function set_localize_data($handle, $var_name, $js_vars=array(), $global_js_vars=array()){
		...
		switch ($handle) {
			...
			case 'nmsf-settings-panel':
				...
				$localize_data = [
        		   'migrate_back_msg' => __('Are you sure?', $this->get_config('plugin_domain'))
    		    ];
